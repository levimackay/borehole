...
on `engine-evidence` and nothing lower, which is what guarantees the CLI and
...
4. Nothing in `engine-index`, `engine-graph`, `engine-git`, or
   `engine-evidence` needs to change, since they're language-agnostic by
...
- Git access is exclusively via `git2` (libgit2), never by shelling out to
  a `git` binary, so a malicious repo cannot inject shell commands through a
...
| Explain (evidence summary) | `borehole explain <path\|symbol>` | None (composed client-side from the above) |
...
- Indexing walks files in parallel (`rayon`) and reports progress via a
  ...
  app can emit a Tauri event, and neither blocks the UI thread.
